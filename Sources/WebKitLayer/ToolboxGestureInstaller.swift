...
final class ToolboxGestureInstaller: NSObject, UIGestureRecognizerDelegate {
    ...
    /// 底边上滑那个要单独认出来做起手位置判断
    private weak var bottomSwipe: UISwipeGestureRecognizer?

    /// 底边判定区高度。太大就会在页面下半部乱触发。
    private let bottomEdgeHeight: CGFloat = 32
    ...
    nonisolated func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        MainActor.assumeIsolated {
            guard gestureRecognizer === bottomSwipe, let host else { return true }
            let location = gestureRecognizer.location(in: host)
            return location.y >= host.bounds.height - bottomEdgeHeight
        }
    }
}
